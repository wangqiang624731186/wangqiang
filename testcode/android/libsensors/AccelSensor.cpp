...
#define ACC_DATA_NAME    "lsm330dlc_acc" 
#define ACC_SYSFS_PATH   "/sys/devices/platform/imx-i2c.1/i2c-1"
#define ACC_SYSFS_DELAY  "poll"
#define ACC_SYSFS_ENABLE "enable_device"
...
void AccelSensor::processEvent(int code, int value)
{
    ...
    switch (code) {
        case ACC_EVENT_X :
            mPendingMask = 1;
            mPendingEvent.acceleration.x =  16 * ACC_DATA_CONVERSION(value);
            break;
        case ACC_EVENT_Y :
            mPendingMask = 1;
            mPendingEvent.acceleration.y = 16 * ACC_DATA_CONVERSION(value);
            break;
        case ACC_EVENT_Z :
            mPendingMask = 1;
            mPendingEvent.acceleration.z = (-16)* ACC_DATA_CONVERSION(value);
            break;
    }
}
...
int AccelSensor::sensor_get_class_path(char *class_path)
{
	...
	closedir(dir);
	ALOGD("sensor_get_class_path:found:%d,class_path:%s\n",found,class_path);
