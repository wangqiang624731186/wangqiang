...
extern int athr_gps_set_ttymxc(int fd);
enum {
  STATE_QUIT  = 0,
  STATE_INIT  = 1,
  STATE_START = 2
...
nmea_reader_parse( NmeaReader*  r )
{
    ...
    */
    ...
    if ( !memcmp(tok.p, "GGA", 3) ) {
		...
		ALOGD("999999999999999999999999999999999999999999\n");
        Token  tok_fixStatus     = nmea_tokenizer_get(tzer,2);
...
{
    int cnt;
    ALOGD("nmea_reader_addc1 \n");

    if (r->overflow) {
        r->overflow = (c != '\n');
    ALOGD("nmea_reader_addc2 \n");
        return;
    }

    ALOGD("nmea_reader_addc3 \n");
    if (r->pos >= (int) sizeof(r->in)-1 ) {
        r->overflow = 1;
        r->pos      = 0;
    ALOGD("nmea_reader_addc4 \n");
        return;
    }
    ...
    ALOGD("nmea_reader_addc5 \n");
    if (c == '\n') {
        gps_state_lock_fix(gps_state);
    ALOGD("nmea_reader_addc6 \n");
        nmea_reader_parse( r );
    ALOGD("nmea_reader_addc7 \n");
        gps_state_unlock_fix(gps_state);
        r->pos = 0;
    }
    ALOGD("nmea_reader_addc8 \n");
}
...
void gps_wakeup(GpsState *state)
{
	...
	if( sleep_lock == 0) // it reset by athr_gps_start
	{
		...
		{
			D("Send WAKEUP command to GPS1\n");
			sleep_lock = time((time_t*)NULL);
			/* send $PUNV,WAKEUP command*/
			D("Send WAKEUP command to GPS2\n");
			...
			{
			D("Send WAKEUP command to GPS3\n");
				bOrionShutdown = 0;
			}
			D("Send WAKEUP command to GPS4\n");
			ALOGD("bOrionShutdown:%d\n",bOrionShutdown);
			D("Send WAKEUP command to GPS5\n");
		}
		bGetFormalNMEA = 0;
...
*/
    ...
    if (state->fd < 0) {
		...
		if ( snprintf(device, sizeof(device), "/dev/%s", prop) >= (int)sizeof(device) ) {
            DFR("gps serial device name too long: '%s'\n", prop);
            return -1;
        }
		...
		if(athr_gps_set_ttymxc(state->fd)== -1)
		{
			D("athr_gps_set_ttymxc failed\n");
			return -1;
		}
		D("athr_gps_set_ttymxc sucess\n");   
...
gps_state_init( GpsState*  state )
{
	...
	if (sem_init(&state->fix_sem, 0, 1) != 0) {
      D("gps semaphore initialization failed! errno = %d\n", errno);
      return;
    }
...
/*    if (property_get("athr.gps.mode",prop,"hosted") == 0)
	...
	strncpy(prop,"ttymxc1",PROPERTY_VALUE_MAX);
    ALOGD("gps dev:%s\n",prop);
...
athr_gps_set_fix_frequency(int freq)
{
    ...
    D("gps fix frquency set to %d secs\n", freq);
}
...
/*	GpsState*  s = _gps_state;
	...
	{
		D("%s: set GPS POSITION mode error! (mode:%d) \n", __FUNCTION__, mode);
    ...
    D("gps fix frquency set to %d sec\n", s->fix_freq);
    switch(s->fix_freq)
    {
        case 1:
